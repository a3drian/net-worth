...
export class ShowDepositDialogComponent implements OnInit {
	...
	save(): void {
		...
		// TODO:
		// open deposit, change "Reimbursable" to "true"
		// on PC, "changed" is "true" when you click "Update"
		// on iOS, it is "false" => de aia si butonul imediat dupa devine disabled, pentru ca "changed" e false
		// daca in schimb modifici "amount", merge si pe PC si pe iOS => ceva in neregula cu checkboxurile
		if (changed) {
	...
	isDetailsValid(): boolean {
		...
		const required = errors['required'];
		if (required) {
			this.detailsErrorMessage = this.detailsErrors.empty;
		}
		const tooLong = errors['maxlength'];
		if (tooLong) {
			this.detailsErrorMessage = this.detailsErrors.tooLong;
		}
		return control.valid;
	}
	...
	private initializeDepositForm(
		...
		const amount = new FormControl<number>(initial.amount, [
			Validators.required,
			Validators.min(0),
			positiveNumberValidator()
		]);
		const currency = new FormControl<string>(initial.currency, [
			Validators.required
		]);
		const details = new FormControl<string>(initial.details, [
			Validators.required,
			Validators.maxLength(30)
		]);
		const createdAt = new FormControl<string>(
			initialDate.toISOString().split('T')[0],
			[Validators.required]
		);
		const category = new FormControl<string>(initial.category, [
			Validators.required
		]);
	...
	private getDifferences(deposit: IDeposit | null): DepositDifferences[] {
		const controls: IControl[] = Object.entries(
			this.depositForm.controls
